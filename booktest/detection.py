...
def parse_booktest_setup(context, root, f):
...
def parse_test_file(context, root, f):
    rv = []
        ...
        for root, dirs, files in os.walk(full_path):
            ...
            for f in files:
                if f == BOOKTEST_SETUP_FILENAME:
                    setup = parse_booktest_setup(context, rel_path, f)

    return setup


def detect_tests(context, path):
    full_path = os.path.join(context, path)
            ...
            for f in files:
                if f.endswith("_test.py") or f.endswith("_book.py") or f.endswith("_suite.py") or \
                   (f.startswith("test_") and f.endswith(".py")):
                    tests.extend(parse_test_file(context, rel_path, f))

    return tests


def detect_test_suite(context, path):
    tests = detect_tests(context, path)
